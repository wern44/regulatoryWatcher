...
`regwatch/discovery/cssf_scraper.py` is a pure-HTTP scraper with no DB access; the reconciliation lives in `regwatch/services/cssf_discovery.py`. Parser correctness is anchored to the HTML fixtures under `tests/fixtures/cssf/` — unit tests parse those files via `BeautifulSoup` and never hit the network. When the CSSF DOM changes, the `tests/live/test_cssf_live_probe.py` suite (marked `@pytest.mark.live`, excluded from default `pytest`) is the canary; refresh the fixtures per `tests/fixtures/cssf/README.md` then update the scraper's CSS selectors. Amendment-graph writes go through `RegulationLifecycleLink` and only come from explicit wording on CSSF pages (`(as amended by …)`, `amending Circular …`, `Update of …`) — a "Related document" alone is not an amendment. Within a run links are only added; at the end of a successful full run `_reconcile_links` deletes links that no crawled page supports when both endpoints were crawled, or when an older document supposedly amends a newer one. The listing row's reference is a regulation's identity (never the detail page's title — annexes name their parent circular). `_refresh_metadata` never overwrites `source_of_truth="SEED"` rows. Entity-slug mapping lives in `regwatch/services/cssf_discovery.py::CSSF_ENTITY_SLUGS` — one slug per `AuthorizationType`.
